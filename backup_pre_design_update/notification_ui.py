    ...
    if not favs:
        ...
        if not filtered_favs:
            ...
            for t, name in visible_tickers:
                ...
                alerts_str = []
                if price_alerts: alerts_str.append(f"価格{len(price_alerts)}")
                if trendlines: alerts_str.append(f"ライン{len(trendlines)}")
                if date_alerts: alerts_str.append(f"日付{len(date_alerts)}")
                
                if alerts_str:
                    status_parts.append("アラート: " + "・".join(alerts_str))
                else:
                    status_parts.append("アラート: なし")
